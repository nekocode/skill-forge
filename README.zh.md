...
| 不知道 skill 写得好不好 | 内置 5 维评估器，不达标不落盘 |
...
| 命令 | 功能 |
|------|------|
| `/scan [focus]` | 扫描项目发现 skill 机会（优先搜社区库） |
| `/create <name>` | 创建新 skill：codebase 研究 + 评估 |
| `/improve <name>` | 迭代现有 skill：内容修补 / 触发优化 / 两者 |
...
4. **Vercel find-skills** — 建之前先搜社区（agentskills.io / skills.sh）

### 双文件安全模型

外部内容（grep/glob/read 输出）写入 `skill_insights.md`（低信任，hooks 不读）。验证合法后才提升到 `skill_draft.md`（高信任，被 hooks 反复注入）。防止 prompt injection 放大。
...
复杂任务完成
  -> Stop hook / 手动调用
  -> scan（社区搜索门控）-> create（草稿 -> 研究 -> SKILL.md -> 评估 >= 6/8）
